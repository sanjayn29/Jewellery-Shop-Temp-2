...
const Navbar = () => {
  ...
  return (
    <>
      {/* Top Bar */}
      <div className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-r from-gold via-amber to-orange text-white text-xs py-2 animate-fade-in">
      ...
      <nav
        className={`fixed top-10 left-0 right-0 z-40 transition-all duration-500 ${
          isScrolled
            ? 'bg-cream/95 backdrop-blur-xl shadow-elegant border-b border-gold/20'
        ...
        <div className="container mx-auto px-6 lg:px-12">
          <div className="flex items-center justify-between h-20 lg:h-24">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="relative">
                  ...
                  <img
                    ...
                    className="h-12 w-12 lg:h-14 lg:w-14 object-contain transition-all duration-500 group-hover:drop-shadow-[0_0_15px_rgba(212,175,55,0.6)] relative z-10"
                  />
                  {/* Subtle rotating ring effect */}
                  <div className="absolute inset-0 rounded-full border-2 border-gold/0 group-hover:border-gold/40 group-hover:scale-110 transition-all duration-700 group-hover:animate-diamond-rotate" />
                </div>
                <div className="flex flex-col">
                  <span className="font-display text-xl lg:text-2xl tracking-wider text-charcoal bg-gradient-to-r from-gold via-amber to-gold bg-clip-text group-hover:text-transparent transition-all duration-500">
                    MEENAKSHI
                  </span>
              ...
              <div className="absolute -bottom-1 -left-1 w-1.5 h-1.5 bg-amber rounded-full opacity-0 group-hover:opacity-100 group-hover:animate-twinkle" style={{ animationDelay: '0.3s' }} />
            </a>
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <span className="font-body text-sm tracking-widest text-charcoal hover:text-gold transition-colors duration-300">
                        {category.name}
                      </span>
                    </a>
                    {/* Gold underline with elegant effect */}
                    <span className="absolute -bottom-2 left-1/2 w-0 h-0.5 bg-gradient-to-r from-gold via-amber to-gold group-hover:w-full group-hover:left-0 transition-all duration-500" />
                        ...
                        <div className="py-3">
                          {category.subcategories.map((sub, idx) => (
                            <a
                              ...
                              className="block px-6 py-3 font-body text-sm text-charcoal hover:bg-gradient-elegant hover:text-gold transition-all duration-300 border-l-2 border-transparent hover:border-gold"
            ...
            <div className="flex items-center space-x-2 lg:space-x-4">
              <button className="hidden lg:flex p-2.5 text-charcoal hover:text-gold transition-all duration-300 hover:scale-110 hover:bg-ivory rounded-full group relative">
                <Search className="w-5 h-5" />
                <span className="absolute inset-0 rounded-full border border-gold/0 group-hover:border-gold/30 transition-all duration-300" />
              </button>
              <button className="hidden lg:flex p-2.5 text-charcoal hover:text-gold transition-all duration-300 hover:scale-110 hover:bg-ivory rounded-full group relative">
                <Heart className="w-5 h-5" />
                <span className="absolute top-0 right-0 w-2 h-2 bg-amber rounded-full animate-twinkle" />
                <span className="absolute inset-0 rounded-full border border-gold/0 group-hover:border-gold/30 transition-all duration-300" />
              </button>
              <button className="hidden lg:flex p-2.5 text-charcoal hover:text-gold transition-all duration-300 hover:scale-110 hover:bg-ivory rounded-full group relative">
              ...
              <button className="relative p-2.5 text-charcoal hover:text-gold transition-all duration-300 hover:scale-110 hover:bg-ivory rounded-full group">
              ...
              {navCategories.map((category, index) => {
                ...
                return (
                  <div key={category.name} className="space-y-3">
                    <a
                      href={category.href}
                      className="flex items-center gap-3 text-lg font-body text-charcoal hover:text-gold transition-colors animate-fade-in"
                      ...
                      <div className="ml-8 space-y-2">
                        {category.subcategories.map((sub) => (
                          <a
                            key={sub}
                            href={`${category.href}/${sub.toLowerCase().replace(' ', '-')}`}
                            className="block text-sm font-body text-charcoal hover:text-gold transition-colors"
                            onClick={() => setIsMobileMenuOpen(false)}
                          >
                            {sub}
                          </a>
